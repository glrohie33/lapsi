  ...
  <div class="app-content container">
    ...
    <div class="content-wrapper">
      <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
          <div class="row breadcrumbs-top">
            <div class="col-12">
              <h2 class="content-header-title float-left mb-0">underwriters</h2>
            </div>
          </div>
        </div>
      </div>
      <div class="content-body">
            ...
            <div class="col-12">
              <div class="card">
                <div class="card-header">
                  <div class="card-title">underwriter</div>
                </div>
                <div class="card-body">
                  <div class="row">
                    <div class="col-md-12">
                      <h3>{{underwriter.registered_name}}</h3>
                    </div>
                    <div class="col-12 col-sm-9 col-md-6 col-lg-5">
                      <table>
                        <tr>
                          <td class="font-weight-bold">Rc number</td>
                          <td>{{underwriter.rc_number}}</td>
                        </tr>
                        <tr>
                          <td class="font-weight-bold">CEO</td>
                          <td>{{underwriter.ceo}}</td>
                        </tr>
                        <tr>
                          <td class="font-weight-bold">City</td>
                          <td>{{underwriter.city}}</td>
                        </tr>
                        <tr>
                          <td class="font-weight-bold">State</td>
                          <td>{{underwriter.state}}</td>
                        </tr>
                      </table>
                    </div>
                    ...
                    <div class="col-12">
                      ...
                      <button
                        ...
                        v-if="permissions('user','update') && underwriter.status == '0'"
                        @click="verify($event,'1',underwriter.id)"
                      >
                        <i class="feather icon-user-check"></i> Approve
                      </button>
                      <button
                        class="btn btn-outline-danger btn-sm"
                        v-if="permissions('user','update') && underwriter.status == '1'"
                        @click="verify($event,'0',underwriter.id)"
                      >
        ...
        <section id="column-selectors" v-show="underwriters">
          <div class="row">
            <div class="col-12">
              <div class="card">
                <div class="card-header">
                  <h4 class="card-title">All underwriterss Table</h4>
                </div>
                          ...
                          <div style="overflow:hidden">
                            ...
                            <div class="form-group" style="float:right;">
                              ...
                              <select v-model="rankFilter" class="form-control select2-icons">
                                <option value="-1" data-icon="fa fa-male">All underwriters</option>
                                <option value="0">Unranked underwriters</option>
                            ...
                            <div class="card">
                              ...
                              <div class="card-content">
                                <div class="card-body">
                                  ...
                                  <p class="text-danger text-center">{{underwriterError}}</p>
                                  <div class="col-12">
                                    <button
                                      type="submit"
                                      @click="addRank($event)"
...
export default {
  ...
  methods: {
    getData() {
      ...
      var sort = sortBy[0];
      var desc = sortDesc[0];
      const url = `${index_url}/api/underwriters?page=${page}&limit=${itemsPerPage}&sort=${sort}&desc=${desc}&search=${this.search}&filter=${this.filter}&rank=${this.rankFilter}`;
      axios.get(url, formHeader()).then(resp => {
        this.items = resp.data.underwriters;
        this.total = resp.data.total;
      });
    },
    getRank() {
      axios.get(`${index_url}/api/underwriterrank`).then(resp => {
        this.ranks = resp.data.rank;
      });
    },
    view(item) {
      this.underwriter = item;
      this.underwriter.rel =
        typeof item.rel == "object" ? item.beneficiaries : JSON.parse(item.rel);
      this.underwriters = false;
      this.viewProfile = true;
    },
    showList() {
      this.underwriters = true;
      this.viewProfile = false;
    },
    setParents() {
      var id = this.underwriter.rank_id;
      var underwriterrank = this.ranks.find(rank => rank.id == id);
      this.parentRank = [];
      this.underwriter.rel = [];
      this.levels = underwriterrank.rank;
      this.currentLevel = 1;
      this.setNextRank();
    },
    setNextRank() {
      if (this.currentLevel < this.levels) {
        var len = this.underwriter.rel.length;
        var id = len > 0 ? this.underwriter.rel[len - 1] : null;
        if (id == null) {
          id = this.ranks[this.currentLevel - 1].id;
          var url = `${index_url}/api/underwriterrankusers?id=${id}`;
        } else {
          var url = `${index_url}/api/underwriterchildren?id=${id}`;
        }
        var rank_id = axios.get(url).then(resp => {
          this.parentRank.push(resp.data.underwriters);
          this.currentLevel += 1;
        });
      }
    },
    verify(event, status, id = null) {
      if (this.selected.length > 0 || id != null) {
        var underwriters = this.selected.map(underwriter => underwriter.id);
        if (id != null) {
          underwriters = [id];
        }
        ...
        axios
          .post(
            `${index_url}/api/underwriter/verify`,
            { underwriters: underwriters, status: status },
            formHeader()
          )
          .then(resp => {
            if (resp.data.status) {
    ...
    addRank(event, id = null) {
      ...
      if (len == this.levels - 1) {
        var parent_id = len > 0 ? this.underwriter.rel[len - 1] : "";
        var selected = this.selected.map(underwriter => underwriter.id);
        var data = {
          underwriters: selected,
          rank_id: this.underwriter.rank_id,
          rel: this.underwriter.rel,
          parent_id: parent_id
        };
        axios
          .post(`${index_url}/api/underwriter/addrank`, data, formHeader())
          .then(resp => {
            if (resp.data.status) {
              Swal.fire({
                title: "Rank Added",
        ...
        this.underwriterError = `This underwriter needs to be under ${this
          .levels - 1} underwriter(s)`;
        button.removeAttribute("disabled");
      }
    }
  }
