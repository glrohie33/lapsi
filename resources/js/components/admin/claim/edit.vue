      ...
      <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
          <div class="row breadcrumbs-top">
            <div class="col-12">
              <h2 class="content-header-title float-left mb-0">Edit Claim</h2>
            </div>
          </div>
        </div>
      </div>
                    ...
                    >
                      <div class="form-body">
                        <div class="row">
                          ...
                          <div class="col-md-12 form-group">
                            <label>Note</label>
                            <textarea type v-model="claim.note" class="form-control" readonly></textarea>
                          </div>
                          <div class="col-md-12">
                            <div class="row">
...
export default {
  ...
  methods: {
    getData() {
      axios
        .get(`${index_url}/api/claim/${this.$route.params.id}/edit`)
        .then(resp => {
          var claim = resp.data.claim;
          claim.files = JSON.parse(claim.files);
          this.claim = claim;
          this.setAssetTypes();
        });
    ...
    editClaim(event) {
      ...
      axios.post(`${index_url}/api/claim/${this.claim.id}`, data).then(resp => {
        if (resp.data.status) {
          Swal.fire({
            title: "Claim Edited",
            text: "You have successfully edited claim data",
            icon: "success"
          });
